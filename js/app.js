...
function normalizeBlocks(blocks){ return Array.isArray(blocks)?blocks.filter(Boolean).map(b=>({...b,slot:b.slot||'afterHeading'})):[]; }
...
function renderCustomBlocks(cfg, preview=false, slot='afterHeading'){ return normalizeBlocks(cfg.customBlocks).filter(b=>(b.slot||'afterHeading')===slot).map(b=>renderCustomBlock(b,preview)).join(''); }
...
window.addSiteBlock=(type,index=null,slot='afterHeading')=>{
  ...
  const block={...defaultSiteBlock(type),slot};
...
function builderPlacedBlocks(cfg,slot){
  const arr=normalizeBlocks(cfg.customBlocks).filter(b=>(b.slot||'afterHeading')===slot);
...
window.dropBuilderAt=(event,slot,beforeId='')=>{
  ...
  if(palette && ['heading','text','announcement','image','button','divider'].includes(palette)){
    const block={...defaultSiteBlock(palette),slot};
...
window.setEditorTab=(tab)=>{ if(!['branding','builder','navigation','labels'].includes(tab)) return; state.editorTab=tab; renderSiteEditor(); };
...
function renderSiteEditor(){
  ...
  const navs=navItems(cfg); const selectedNav=navs.find(x=>x.id===state.editorSelectedNav)||navs[0]; state.editorSelectedNav=selectedNav?.id||'';
  const navigationSection=`<div class="editorSection"><div class="navEditorHeader"><div><h3>Navigation / Tabs</h3><p class="small">Rename, reorder, show/hide tabs, and edit the content shown inside each page.</p></div><button onclick="addCustomNavTab()">＋ Add Tab</button></div><div class="navEditorList">${navs.map((n,i)=>`<div class="navEditorRow ${selectedNav?.id===n.id?'selected':''}"><span class="navDragDots">⠿</span><input value="${esc(n.label)}" oninput="updateNavItem('${n.id}','label',this.value)"><select onchange="updateNavItem('${n.id}','access',this.value)"><option value="everyone" ${n.access==='everyone'?'selected':''}>Everyone</option><option value="coordinator" ${n.access==='coordinator'?'selected':''}>Coordinator only</option></select><button class="tinyBtn" onclick="moveNavItem('${n.id}',-1)" ${i===0?'disabled':''}>↑</button><button class="tinyBtn" onclick="moveNavItem('${n.id}',1)" ${i===navs.length-1?'disabled':''}>↓</button><button class="tinyBtn" onclick="toggleNavItem('${n.id}')">${n.hidden?'Show':'Hide'}</button><button class="tinyBtn" onclick="selectNavEditor('${n.id}')">Edit Content</button>${n.custom?`<button class="tinyBtn dangerLite" onclick="deleteCustomNavTab('${n.id}')">Delete</button>`:''}</div>`).join('')}</div></div>${selectedNav?`<div class="editorSection navContentEditor"><h3>Edit “${esc(selectedNav.label)}” Content</h3><p class="small">For built-in functional tabs, this content appears above the app controls. Custom tabs use this as the page content.</p><label>Page Heading</label><input value="${esc(selectedNav.heading||'')}" oninput="updateNavItem('${selectedNav.id}','heading',this.value)"><label>Subtitle</label><textarea oninput="updateNavItem('${selectedNav.id}','subtitle',this.value)">${esc(selectedNav.subtitle||'')}</textarea><label>Page Content</label><textarea class="navBodyField" oninput="updateNavItem('${selectedNav.id}','body',this.value)">${esc(selectedNav.body||'')}</textarea></div>`:''}`;
  ...
  const activeSection=tab==='builder'?builderSection:tab==='navigation'?navigationSection:tab==='labels'?labelsSection:brandingSection;
  $('#main').innerHTML=`<section class="siteEditorShell"><div class="siteEditorToolbar"><div><h2>Visual Site Editor <span class="versionChip">v3.5.1</span></h2><p>Edit, preview, save as draft, then publish when ready.</p></div><div class="editorToolbarActions"><button class="secondary" onclick="saveSiteDraft()">Save Draft</button><button class="secondary" onclick="discardSiteChanges()">Discard</button><button class="secondary" onclick="resetSiteDraft()">Reset</button><button class="publishBtn" onclick="publishSiteChanges()">Publish Changes</button></div></div><div class="siteEditorGrid"><div class="editorPanel"><div class="editorTabs"><button class="${tab==='branding'?'editorTabActive':''}" onclick="setEditorTab('branding')">Branding</button><button class="${tab==='builder'?'editorTabActive':''}" onclick="setEditorTab('builder')">Page Builder</button><button class="${tab==='navigation'?'editorTabActive':''}" onclick="setEditorTab('navigation')">Navigation</button><button class="${tab==='labels'?'editorTabActive':''}" onclick="setEditorTab('labels')">Labels</button></div>${activeSection}</div><div class="previewPanel"><div class="previewPanelHead"><div><b>Live Preview</b><span>Draft only — not live yet</span></div><div><button data-mode="desktop" class="previewModeBtn ${state.editorPreviewMode==='desktop'?'active':''}" onclick="setEditorPreviewMode('desktop')">Desktop</button><button data-mode="mobile" class="previewModeBtn ${state.editorPreviewMode==='mobile'?'active':''}" onclick="setEditorPreviewMode('mobile')">Mobile</button></div></div><div class="sitePreviewFrame" data-mode="${state.editorPreviewMode}"><div id="siteEditorPreview"></div></div></div></div></section>`;
  renderSiteEditorPreview();
}
