...
const DEFAULT_SITE_SETTINGS = {
  ...
  bookedStatusLabel:'Court Booked',
  renovationStatusLabel:'Closed for Renovation',
...
function statusBadges(ev){
  const list=selectedStatuses(ev), cfg=siteSettings();
  return list.length ? list.map(s=>`<span class="badge ${s.cls}">${s.icon} ${esc(s.label)}</span>`).join('') : `<span class="badge yellow">${esc(cfg.waitingStatusLabel)}</span>`;
}
function eventStatus(ev){ const list=selectedStatuses(ev), cfg=siteSettings(); return list[0] || {key:'waiting',label:cfg.waitingStatusLabel,cls:'yellow',icon:'⏳'}; }
...
function statusNotificationData(ev,data,isNew){
  ...
  const keys=[
    ['booked','booked',`${loc}: ${cfg.bookedStatusLabel}`,`${dateLabel} • ${time}. ${data.details||ev?.details||''}`],
    ['full','full',`${loc}: ${cfg.fullStatusLabel}`,`${dateLabel} • ${time}.`],
    ['closed','renovation',`${loc}: ${cfg.renovationStatusLabel}`,`${data.details||ev?.details||'This location/event is temporarily closed for renovation.'}`],
    ['closedEvent','closedEvent',`${loc}: ${cfg.closedEventStatusLabel}`,`${data.details||ev?.details||'This location is closed for a scheduled event.'}`],
    ['cancelled','cancelled',`${loc}: ${cfg.cancelledStatusLabel}`,`${data.details||ev?.details||'This play date has been cancelled.'}`]
  ];
...
function renderGuestEvent(){
 ...
 const statusBox=ev.cancelled?`<div class="notice warn"><b>${esc(cfg.cancelledStatusLabel)}</b></div>`:ev.closedEvent?`<div class="notice warn"><b>${esc(cfg.closedEventStatusLabel)}</b></div>`:ev.closed?`<div class="notice warn"><b>${esc(cfg.renovationStatusLabel)}</b></div>`:ev.full?`<div class="notice warn"><b>${esc(cfg.fullStatusLabel)}</b></div>`:ev.booked?`<div class="notice"><b>${esc(cfg.bookedStatusLabel)}</b><br>${esc(ev.details||cfg.bookedStatusLabel)}</div>`:`<div class="notice warn"><b>${esc(cfg.waitingTitle)}</b><br>${esc(waitingMessageText(cfg))}</div>`;
...
function playerEventInner(ev, opts={}){
 ...
 const statusBox = (()=>{
  if(ev.cancelled) return `<div class="notice warn"><b>${esc(cfg.cancelledStatusLabel)}</b><br>Please see coordinator details below.</div>`;
  if(ev.closedEvent) return `<div class="notice warn"><b>${esc(cfg.closedEventStatusLabel)}</b><br>Please see coordinator details below.</div>`;
  if(ev.closed) return `<div class="notice warn"><b>${esc(cfg.renovationStatusLabel)}</b><br>Please see coordinator details below.</div>`;
  if(ev.full) return `<div class="notice warn"><b>${esc(cfg.fullStatusLabel)}</b><br>This play date is currently full.</div>`;
  if(ev.booked) return `<div class="notice"><b>${esc(cfg.bookedStatusLabel)}</b><br>${esc(ev.details||cfg.bookedStatusLabel)}</div>`;
 ...
 const closedReason = ev.cancelled?cfg.cancelledStatusLabel:ev.closedEvent?cfg.closedEventStatusLabel:ev.closed?cfg.renovationStatusLabel:(ev.full||st.key==='full')?cfg.fullStatusLabel:'Signup closed';
