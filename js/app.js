...
function eventCardPlayer(ev){
 ...
 const statusBox = ev.booked
  ? `<div class="notice"><b>Booking Details</b><br>${esc(ev.details||'Court booked. Details coming soon.')}</div>`
  : `<div class="notice warn"><b>${ev.closed?'Closed for renovation.':'Waiting for court reservation.'}</b><br>${ev.closed?'Please see coordinator details below.':'Minimum 6 playing players before booking court.'}</div>`;
 const detailBox = (!ev.booked && ev.details) ? `<div class="notice info"><b>📢 Coordinator Details</b><br>${esc(ev.details)}</div>` : '';
 return `<div class="card"><div class="eventTop"><div><div class="big">${fmtDate(ev.date)}</div><p>📍 <b>${esc(ev.location)}</b><br>🕒 ${timeLabel(ev.start)} - ${timeLabel(ev.end)}</p></div><div>${c.playing>=6?'<span class="badge red">6+ Ready</span>':''}<span class="badge ${st.cls}">${st.label}</span></div></div>${statusBox}${detailBox}${ev.feeOn?feeHtml(ev):''}<h3>Sign up / Update my family</h3>${closed?`<p><span class="badge red">${st.label==='FULLY BOOKED'?'Fully booked':'Closed for renovation'}</span></p>`:family.map(n=>{const ex=mine.find(s=>s.name===n);return `<div class="person"><div><b>${esc(n)}</b><div class="small">${ex?esc(ex.status):'Not signed up'}</div></div><div class="actions"><button class="secondary" onclick="upsertSignup('${ev.id}','${idSafe(n)}','interested')">Interested</button><button class="success" onclick="upsertSignup('${ev.id}','${idSafe(n)}','playing')">Playing</button>${ex?`<button class="danger" onclick="removeMySignup('${ev.id}','${idSafe(n)}')">Remove</button>`:''}</div></div>`}).join('')}<h3>Who’s Coming</h3>${signups.length?signups.map(s=>`<div class="person"><span>${s.status==='playing'?'✅':'👍'} <b>${esc(s.name)}</b> <span class="small">${esc(s.status)}</span></span></div>`).join(''):'<p class="small">No signups yet.</p>'}</div>`;
}
