...
let state = { user:null, profile:null, events:[], locations:[], notifications:[], showNotifications:false, view:localStorage.getItem('pickleballView')||'player', ready:false, calendarDate:new Date(), selectedCalendarDate:today() };
let unsubscribers = [];
...
function calendarMonthTitle(d){ return d.toLocaleDateString(undefined,{month:'long',year:'numeric'}); }
...
function renderCalendar(){
 ...
 const eventDates=new Set(state.events.map(e=>e.date).filter(Boolean));
 let cells='';
 for(let i=0;i<firstDay;i++) cells+=`<button class="calDay empty" disabled></button>`;
 for(let day=1;day<=daysInMonth;day++){
   const key=dateKey(year,month,day);
   const has=eventDates.has(key);
   const selected=state.selectedCalendarDate===key;
   const isToday=key===today();
   cells+=`<button class="calDay ${has?'hasEvent':''} ${selected?'selected':''} ${isToday?'today':''}" onclick="selectCalendarDate('${key}')"><span>${day}</span>${has?'<i></i>':''}</button>`;
 }
 const selectedEvents=[...state.events].filter(e=>e.date===state.selectedCalendarDate).sort((a,b)=>(a.start||'').localeCompare(b.start||''));
 $('#main').innerHTML=`<div class="calendarShell"><div class="calendarHeader"><button class="secondary" onclick="changeCalendarMonth(-1)">‹</button><h2>${calendarMonthTitle(base)}</h2><button class="secondary" onclick="changeCalendarMonth(1)">›</button></div><div class="calendarWeekdays"><span>Sun</span><span>Mon</span><span>Tue</span><span>Wed</span><span>Thu</span><span>Fri</span><span>Sat</span></div><div class="calendarGrid">${cells}</div></div><div class="card calendarDetails"><h2>${state.selectedCalendarDate?fmtDate(state.selectedCalendarDate):'Select a date'}</h2>${selectedEvents.length?selectedEvents.map(ev=>{const c=eventCounts(ev);return `<div class="calendarEventInfo"><div><b>${esc(ev.location)}</b><p>🕒 ${timeLabel(ev.start)} - ${timeLabel(ev.end)}</p><p>${statusBadges(ev)}</p><p class="small">${c.playing} playing • ${c.interested} interested</p>${ev.details?`<p>${esc(ev.details)}</p>`:''}</div><button onclick="openCalendarEvent('${ev.id}')">View Event</button></div>`}).join(''):'<p class="small">No events on this date.</p>'}</div>`;
}
window.changeCalendarMonth=(delta)=>{const d=new Date(state.calendarDate);d.setDate(1);d.setMonth(d.getMonth()+delta);state.calendarDate=d;renderCalendar();};
window.selectCalendarDate=(key)=>{state.selectedCalendarDate=key;renderCalendar();};
window.openCalendarEvent=(id)=>{const ev=state.events.find(e=>e.id===id);if(!ev)return;state.view='player';localStorage.setItem('pickleballView','player');render();setTimeout(()=>{document.querySelector(`[data-event-id="${id}"]`)?.scrollIntoView({behavior:'smooth',block:'start'});},50);};
